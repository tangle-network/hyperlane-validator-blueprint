...
if [ ! -f "target/debug/hyperlane-validator-blueprint-bin" ]; then
...
if [ "$run_blueprint" = "y" ]; then
    ...
    RUST_LOG=blueprint-rejection=trace,tangle-producer=debug,tangle-consumer=trace,blueprint-router=trace,blueprint-runner=trace,hyperlane_validatorblueprint=debug,hyperlane_validator_blueprint_bin=debug\
    ./target/debug/hyperlane-validator-blueprint-bin run --protocol tangle\
         --blueprint-id 0\
         --service-id 0\
